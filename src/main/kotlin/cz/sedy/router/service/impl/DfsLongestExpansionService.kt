...
@Service
@ConditionalOnProperty(prefix = "dfs", name = ["strategy"], havingValue = "longest")
class DfsLongestExpansionService: DfsExpansionService {
    ...
    override fun expandBorders(
        ...
        countryMap.remove(route.last())
            ...
            ?.sortedBy { country -> country.distance(destinationCountry) }
            ?.map(Country::code)
